...
/**
 * Chat API v2 - Powered by Vercel AI SDK
 *
 * Provides chat completions with optimized settings for business analytics
 * and financial reporting. Maintains backward compatibility with v1 API.
 *
 * @route POST /api/chat-v2
 * @route GET /api/chat-v2 - Health check
 * @route OPTIONS /api/chat-v2 - CORS preflight
 */
...
const DEFAULT_OPTIONS = {
  ...
  max_tokens: 2000, // Sufficient for detailed reports
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (stream) {
      ...
      // Convert to the same SSE format as the original route
    ...
    // Return response in the same format as the original route
